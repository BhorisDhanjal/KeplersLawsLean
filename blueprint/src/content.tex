...
\uses{def:ellipse-data,thm:basic-identities,thm:lrl-const,thm:angular-momentum-const}
...
The origin is a focus.
\uses{thm:cartesian-ellipse,thm:ellipse-parameters}
...
\end{itemize}
\uses{thm:ellipse-parameters,thm:orbit-equation,thm:cartesian-ellipse,thm:first-law-focus}
...
Since $x(t) = a(\cos\theta(t) - e)$ and $y(t) = b\sin\theta(t)$ together determine $q(t)$ in the orbital plane, the angle $\theta(t)$ determines $q(t)$.
Note that for the eccentric anomaly, $r(t) = a(1 - e\cos\theta(t))$.
...
Since $0 \le e < 1$, we have $1 - e\cos\theta > 0$, so it follows that
\[
  \dot\theta = \frac{\|L\|}{mab(1 - e\cos\theta)} > 0.
\]
